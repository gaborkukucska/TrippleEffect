// START OF FILE static/js/app.js (Bottom Navigation Version - Send Fix)

// --- WebSocket Connection ---
let ws; // WebSocket instance - This needs to be accessible by send function
...
// --- Initialization ---
// (DOMContentLoaded remains the same - finds elements, initializes UI, calls setupWebSocket, setupEventListeners)
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM fully loaded. Initializing Bottom Nav UI...");
    // Get Elements
    chatView = document.getElementById('chat-view'); /* ... etc ... */
...
// --- WebSocket Setup ---
// (setupWebSocket remains the same - it assigns to the global `ws`)
...
function setupEventListeners() {
    // Input listeners
    sendButton?.addEventListener('click', handleSendMessage); // Calls the correct handler
    ...
    navButtons?.forEach(button => {
        button.addEventListener('click', () => {
            ...
            showView(viewId); // Use the navigation function
        });
    });
...
function showView(viewId) {
    ...
    else { console.error(`View panel ${viewId} not found!`); document.getElementById('chat-view')?.classList.add('active'); viewId = 'chat-view'; } // Fallback
 ...
 * CRITICAL: Uses the global `ws` instance.
 */
function handleSendMessage() {
    const messageText = messageInput?.value.trim() ?? '';
    if (!messageText && !currentFile) { console.log("Empty message/no file."); return; }

    // --- Check WebSocket state ---
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        addMessage('system-log-area', '[SysErr] WebSocket not connected. Cannot send.', 'error');
        console.error("WebSocket not open. State:", ws ? ws.readyState : 'null');
        // Optionally try to reconnect? Or just inform user.
        // setupWebSocket(); // Be careful with immediate reconnects here
        return;
    }
    // --- End Check ---
    ...
    if (currentFile && currentFileContent) {
        ...
        messageToSend.file_content = currentFileContent;
         ...
         console.error("File selected but no content."); addMessage('system-log-area', '[UI Err] File content missing.', 'error'); clearFileInput(); return;
    }
    ...
    try {
        ...
        console.log(`Sending WS message: ${messageString.substring(0,100)}...`); // Log before sending
        ws.send(messageString);
         // Add confirmation to log? Optional.
         // addMessage('system-log-area', `[UI->Backend] Sent message.`, 'status');
    } catch (error) {
        console.error("Error sending WS message:", error);
        addMessage('system-log-area', '[SysErr] Failed to send message.', 'error');
        // Don't clear input if send failed
        return;
    }
    // --- End Send ---

    // Clear input only if send was successful
...
// --- File Handling ---
// (handleFileSelect, displayFileInfo, clearFileInput remain the same)
...
// (displayAgentConfigurations, handleSaveAgent, handleDeleteAgent remain the same)
...
// --- Modal Handling ---
// (openModal, closeModal, showOverrideModal, handleSubmitOverride remain the same)
...
console.log("Main app.js execution finished (Bottom Nav Version - Complete).");
