// START OF FILE static/js/app.js (Bottom Navigation Version - Color/Scroll Fix COMPLETE)
...
function setupWebSocket() {
    ...
    ws = new WebSocket(wsUrl); // Assign to global ws variable
 ...
 * Handles scrolling and applies log color coding.
 * @param {string} areaId - 'conversation-area' or 'system-log-area'.
 * @param {string} text - The message text.
 * @param {string} [type='status'] - Message type ('user', 'agent_response', 'status', 'error').
...
function addMessage(areaId, text, type = 'status', agentId = null) {
    ...
    // Base classes
    messageDiv.classList.add('message', type); // type handles error styling primarily
    if (agentId) messageDiv.dataset.agentId = agentId; // For potential agent-specific styles

    // --- Add Color Coding Classes for System Logs ---
    if (areaId === 'system-log-area') {
        ...
        if (type !== 'error') {
            if (text.includes("Executing tool") || text.includes("Tool result") || text.includes("Manager Result for") || text.includes("validated. Signaling manager")) {
                messageDiv.classList.add('log-tool-use'); // Greenish text
            } else if (text.startsWith("[From @")) {
                messageDiv.classList.add('log-agent-message'); // Bluish text
            }
            // Keep default (likely greyish) color for other status messages
        }
    }
    // --- End Color Coding ---
    ...
    // Scroll the area after a short delay
...
function setupEventListeners() {
    // Input listeners
    sendButton?.addEventListener('click', handleSendMessage); // Calls the correct handler below
...
function handleSendMessage() {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        ...
        return; // Don't proceed if WS not open
    }
    // --- End Check ---
