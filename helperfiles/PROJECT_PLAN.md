...
**Fix 1:** Modified `send_message` constraints to allow combinations with `request_state` (meaning an agent can send a report and transition in one turn). For invalid combos, the framework executes the other tools and returns an instructive error for the message tool. A circuit breaker was added to auto-execute `send_message` after 3 consecutive failures to break persistent loops. *Update (May 2026):* The error message was significantly enhanced to teach agents that they must let their action tools finish and return results first, before sending a message in the *next* turn. The `send_message` documentation was also updated to explicitly warn against multi-tooling.
