...
- **Modular Tool Help System**: Major tools (`FileSystemTool`, `ProjectManagementTool`, `ManageTeamTool`, `WebSearchTool`, `AssetSearchTool`) implement segmented `get_detailed_usage(sub_action=...)` methods. When called without a `sub_action`, they return a concise summary of all available actions. When called with a specific `sub_action` (e.g., `"read"`, `"add_task"`, `"create_agent"`), they return only the relevant action's detailed parameter documentation. **Note on Formatting:** When constructing these help strings using Python f-strings, developers must properly escape any literal JSON braces (`{{` and `}}`) to prevent `KeyError` crashes during the `PromptAssembler`'s `.format()` calls.
...
- **Web Search Integration**: The `WebSearchTool` integrates deeply with **SearXNG** (configured via `.env`) as the primary open-source intelligence gathering mechanism, falling back to a custom DuckDuckGo scraper if the self-hosted engine is unavailable. It now also includes a fully-fledged HTML web scraper to extract readable text content and top outgoing links from specific URLs.
- **Asset Search Integration**: The `AssetSearchTool` enables agents to search and directly download royalty-free assets (sounds, 3D models, icons, images, textures) from numerous open public APIs (like Freesound, Unsplash, Poly Pizza) seamlessly into the shared workspace.
...
- **Workflow Resilience and Error Feedback**: If an agent outputs a trigger but the workflow execution fails (e.g., malformed XML structure within the block like the PM Kickoff Plan), the framework intercepts the `WorkflowResult(success=False)` event. It injects a targeted `[Framework Feedback: Workflow Execution Error]` message directly into the agent's history and tracks consecutive failures. After 3 consecutive workflow failures, the agent is forcefully halted to an `ERROR` state to prevent infinite loops and token waste.
