...
* `WebSearchTool` — 2 actions (`search`, `get_page`). **Note**: Integrated with local SearXNG engine, includes an HTML web scraper.
* `AssetSearchTool` — 3 actions (`search`, `download`, `list_sources`).
