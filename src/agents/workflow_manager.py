...
class AgentWorkflowManager:
    ...
    def get_system_prompt(self, agent: 'Agent', manager: Optional[Any] = None) -> str:
        ...
        if not hasattr(agent, 'state') or not agent.state:
            ...
            default_conv_state = DEFAULT_STATE # Use default state
            ...
            if not prompt_key:
                logger.warning(f"WorkflowManager: No specific prompt key found for agent type '{agent.agent_type}' in state '{agent.state}'. Falling back to default conversation prompt for type.")
                # Fallback logic: Try default conversation prompt for type, then absolute default
                default_conv_state = DEFAULT_STATE # Use general conversation as fallback
                prompt_key = self._prompt_map.get((agent.agent_type, default_conv_state))
                if not prompt_key:
                    logger.error(f"WorkflowManager: No default conversation prompt found for type '{agent.agent_type}'. Using absolute default.")
                    return settings.DEFAULT_SYSTEM_PROMPT
        ...
        formatting_context = {
            ...
            # Adds other common placeholders
        }
        if manager:
            ...
            # --- Adds task_description/plan_description for PM in MANAGE state ---
            ...
            # --- END ---
            # Adds tool descriptions if needed by the prompt template
        ...
        try:
            # --- Get personality instructions for Admin AI ---
