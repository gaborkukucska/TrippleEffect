...
class AgentCycleHandler:
    ...
    *** Records performance metrics for each LLM cycle attempt. ***
    """
    ...
    async def run_cycle(self, agent: Agent, retry_count: int = 0):
        ...
        Handles events, tool execution, errors, retries, and reactivation logic.
        *** Records success/failure and duration for the LLM call attempt. ***
        ...
        logger.info(f"CycleHandler: Starting cycle for Agent '{agent_id}' (Retry: {retry_count}). Provider: {agent.provider_name}, Model: {agent.model}") # Log provider/model
        ...
        # --- *** NEW: Timing and Metric Recording Variables *** ---
        ...
        try:
            ...
            while True:
                ...
                # Handle Error Event (check fatal/transient)
                    ...
                    if is_fatal_error: logger.warning(f"CycleHandler: Detected potentially fatal error indicator for agent '{agent_id}'.")
                    if not is_fatal_error: is_retryable_transient_error = any(ind.lower() in last_error_content.lower() for ind in ["RateLimitError", "Status 429", "Status 503", "APITimeoutError", "timeout error"])
                    current_cycle_error = True
                    break
             ...
             current_cycle_error = True
             is_fatal_error = True # Treat this as fatal
             last_error_content = f"[Manager Error: Unexpected error in generator handler - {e}]"
             # Try to send error to UI, but don't let it block recording metrics
            ...
            call_success = not current_cycle_error # Success if no error occurred during this cycle attempt

            logger.debug(f"Cycle outcome for {agent.provider_name}/{agent.model}: Success={call_success}, Duration={llm_call_duration_ms:.2f}ms")
            # Record the metrics asynchronously
            try:
                 # Access tracker via manager reference
                 ...
                 logger.debug(f"Successfully recorded metrics for {agent.provider_name}/{agent.model}")
            except Exception as record_err:
                 # Log error but don't let it interfere with agent state logic
                 logger.error(f"Failed to record performance metrics for {agent.provider_name}/{agent.model}: {record_err}", exc_info=True)
            ...
            if current_cycle_error:
                ...
                elif is_retryable_transient_error: # Max retries reached
                ...
                else: # Other non-fatal, non-transient errors
