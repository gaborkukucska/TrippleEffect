...
# --- REMOVED AGENT_STATUS_AWAITING_USER_OVERRIDE ---
...
class Agent:
    """
    Represents an individual LLM agent. Handles prompt setup, LLM interaction,
    XML tool parsing, and status tracking. Now relies on AgentManager/CycleHandler
    for error handling beyond basic provider issues.
    """
    def __init__(
        self,
        ...
        if not self.final_system_prompt:
             logger.error(f"Agent {self.agent_id}: 'system_prompt' is missing or empty!")
        ...
        self.agent_config: Dict[str, Any] = agent_config # Store full entry
        ...
        if self.manager and self.manager.tool_executor and self.manager.tool_executor.tools:
            ...
            else: logger.info(f"Agent {self.agent_id}: No tools found, XML parsing disabled.")
        else: logger.warning(f"Agent {self.agent_id}: Manager or ToolExecutor not available, XML parsing disabled.")
    ...
    def set_status(self, new_status: str, tool_info: Optional[Dict[str, str]] = None):
        ...
        # Remove check against AGENT_STATUS_AWAITING_USER_OVERRIDE if needed,
        # but primarily the logic using it will be removed elsewhere.
    ...
    def _find_and_parse_tool_calls(self) -> List[Tuple[str, Dict[str, Any], Tuple[int, int]]]:
        ...
        buffer_content = self.text_buffer; logger.debug(f"Agent {self.agent_id}: Checking buffer for tool calls (Length: {len(buffer_content)}): '{buffer_content[-500:]}'")
        ...
        def parse_single_match(match, is_markdown):
             ...
             try:
                 ...
                 inner_match = self.raw_xml_tool_call_pattern.search(xml_block_to_parse) if self.raw_xml_tool_call_pattern else None # Added check
        ...
        if self.markdown_xml_tool_call_pattern:
            for m in self.markdown_xml_tool_call_pattern.finditer(buffer_content): parsed = parse_single_match(m, True);
                if parsed: markdown_matches.append(parsed)
        if self.raw_xml_tool_call_pattern:
             for m in self.raw_xml_tool_call_pattern.finditer(buffer_content): parsed = parse_single_match(m, False);
    ...
    async def process_message(self) -> AsyncGenerator[Dict[str, Any], Optional[List[ToolResultDict]]]:
        ...
        # --- Removed check for AGENT_STATUS_AWAITING_USER_OVERRIDE ---
        ...
        try:
            ...
            provider_stream = self.llm_provider.stream_completion(
                messages=self.message_history, model=self.model,
                temperature=self.temperature, **self.provider_kwargs
            )
            ...
            logger.debug(f"Agent {self.agent_id}: Provider stream finished. Stream had error: {stream_had_error}. Processing final buffer.")
            ...
            # --- Don't set to IDLE if error occurred; let CycleHandler manage state ---
            if self.status == AGENT_STATUS_PROCESSING and not stream_had_error:
                self.set_status(AGENT_STATUS_IDLE)
            self.text_buffer = ""
            logger.info(f"Agent {self.agent_id}: Finished processing cycle attempt. Final Status was: {self.status}")
