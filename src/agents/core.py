            ...
            for match in self.markdown_xml_tool_call_pattern.finditer(buffer_content):
                ...
                try:
                    ...
                    for param_name_lower, param_value in tool_args.items():
                         if param_name_lower not in expected_params: logger.warning(f"Agent {self.agent_id}: Unknown parameter '{param_name_lower}' provided for fenced XML tool '{tool_name}'. Ignoring."); continue
                         expected_type = expected_params[param_name_lower]['type']; original_param_name = expected_params[param_name_lower]['name']
                         try:
                         ...
                         except ValueError: logger.warning(f"Agent {self.agent_id}: Invalid value type for fenced XML parameter '{param_name_lower}' (expected {expected_type}) in tool '{tool_name}'. Value: '{param_value}'"); valid_structure = False; break
                ...
                except Exception as parse_err: logger.error(f"Agent {self.agent_id}: Error parsing params for fenced XML '{xml_block[:100]}...': {parse_err}", exc_info=True)
        ...
        if self.raw_xml_tool_call_pattern:
             for match in self.raw_xml_tool_call_pattern.finditer(buffer_content):
                 ...
                 try:
                     ...
                     validated_args = {}; valid_structure = True; tool_schema = self.manager.tool_executor.tools[tool_name].get_schema(); expected_params = {p['name'].lower(): p for p in tool_schema.get('parameters', [])}
                     for param_name_lower, param_value in tool_args.items():
                          if param_name_lower not in expected_params: logger.warning(f"Agent {self.agent_id}: Unknown parameter '{param_name_lower}' provided for raw XML tool '{tool_name}'. Ignoring."); continue
                          expected_type = expected_params[param_name_lower]['type']; original_param_name = expected_params[param_name_lower]['name']
                          try:
                              if expected_type == 'integer': validated_args[original_param_name] = int(param_value)
                              elif expected_type == 'float': validated_args[original_param_name] = float(param_value)
                          ...
                          except ValueError: logger.warning(f"Agent {self.agent_id}: Invalid value type for raw XML parameter '{param_name_lower}' (expected {expected_type}) in tool '{tool_name}'. Value: '{param_value}'"); valid_structure = False; break
                     if not valid_structure: continue
                     missing_required = [];
                     for p_name_lower, p_info in expected_params.items():
                          if p_info.get('required', True) and p_info['name'] not in validated_args: missing_required.append(p_info['name'])
