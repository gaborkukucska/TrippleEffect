...
# --- Constants (Added PLANNING status) ---
AGENT_STATUS_IDLE = "idle"
AGENT_STATUS_PROCESSING = "processing"
AGENT_STATUS_PLANNING = "planning" # New status
...
class Agent:
    ...
    def set_status(self, new_status: str, tool_info: Optional[Dict[str, str]] = None, plan_info: Optional[str] = None):
        ...
        # Clear other state info when setting a new status
        self.current_tool_info = tool_info if new_status == AGENT_STATUS_EXECUTING_TOOL else None
        self.current_plan = plan_info if new_status == AGENT_STATUS_PLANNING else None # Set plan if planning
    ...
    # --- Main Processing Logic (Modified to handle planning phase) ---
    async def process_message(self) -> AsyncGenerator[Dict[str, Any], Optional[List[ToolResultDict]]]:
        ...
        Yields:
            Dict[str, Any]: Events ('response_chunk', 'plan_generated', 'tool_requests', 'final_response', 'error', 'status').
        Receives:
        ...
        if not self.manager.tool_executor: logger.error(f"Agent {self.agent_id}: Tool executor unavailable."); # Allow processing but log error
        ...
        try:
            ...
            if not stream_had_error:
                # --- ADDED: Check for Plan Tag First (Primarily for Admin AI) ---
                plan_match = self.plan_pattern.search(self.text_buffer.strip())
                if plan_match:
                    ...
                    # Append the response containing the plan to history
                    ...
                    # Stop processing this turn, wait for approval/reactivation by CycleHandler
                    return
                # --- END Plan Check ---

                # --- Check for Tool Calls (if no plan was found) ---
                parsed_tool_calls = []
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        if self.status == AGENT_STATUS_PLANNING and self.current_plan: state["current_plan"] = self.current_plan # Include plan in state if planning
        return state
