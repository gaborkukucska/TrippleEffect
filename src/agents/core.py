...
class Agent:
    ...
    def __init__(
        ...
        self.plan_pattern = re.compile(PLAN_TAG_PATTERN, re.IGNORECASE | re.DOTALL)
        if self.manager and self.manager.tool_executor and self.manager.tool_executor.tools:
            ...
            if tool_names:
                safe_tool_names_lower = [re.escape(name.lower()) for name in tool_names]; tool_names_pattern_group_lower = '|'.join(safe_tool_names_lower)
                raw_pattern_str = rf"<({tool_names_pattern_group_lower})>([\s\S]*?)</\1>"; self.raw_xml_tool_call_pattern = re.compile(raw_pattern_str, re.IGNORECASE | re.DOTALL)
                md_xml_pattern_str = MARKDOWN_FENCE_XML_PATTERN.format(tool_names=tool_names_pattern_group_lower); self.markdown_xml_tool_call_pattern = re.compile(md_xml_pattern_str, re.IGNORECASE | re.DOTALL | re.MULTILINE)
                logger.info(f"Agent {self.agent_id}: Compiled XML tool patterns for tools: {tool_names}"); logger.debug(f"Agent {self.agent_id}: Raw XML pattern: {self.raw_xml_tool_call_pattern.pattern}")
    ...
    def set_status(self, new_status: str, tool_info: Optional[Dict[str, str]] = None, plan_info: Optional[str] = None):
        ...
        self.status = new_status; self.current_tool_info = tool_info if new_status == AGENT_STATUS_EXECUTING_TOOL else None; self.current_plan = plan_info if new_status == AGENT_STATUS_PLANNING else None
    ...
    async def process_message(self) -> AsyncGenerator[Dict[str, Any], Optional[List[ToolResultDict]]]:
        ...
        try:
            ...
            # --- Initialize content variable before loop ---
            ...
            async for event in provider_stream:
                ...
                if event_type == "response_chunk":
                     ...
                     # --- End Assignment ---
                     if content: # Check if content is not empty/None
                         self.text_buffer += content; complete_assistant_response += content; yield {"type": "response_chunk", "content": content}
            ...
            if not stream_had_error:
                plan_match = self.plan_pattern.search(self.text_buffer.strip()) if self.plan_pattern else None # Add check for pattern existence
                if plan_match:
