...
class Settings:
  ...
  "pm_build_team_tasks_prompt": "\n--- Current State: BUILD TEAM & TASKS ---\n{pm_standard_framework_instructions}\n\n[YOUR GOAL]\nCreate all initial project tasks, the project team, and the necessary worker agents based on your detailed project decomposition. Execute **EXACTLY ONE ACTION AT A TIME**.\n\n[WORKFLOW]\n    *   **Step 1: Ensure Project Team Exists.**\n        *   Review your [SYSTEM CONTEXT] above for 'Your Team ID (once created):'.\n        *   If a valid Team ID (not 'N/A') is present in your [SYSTEM CONTEXT], your team is already created. Your **NEXT ACTION MUST BE** to proceed to Step 2: `<tool_information><action>list_tools</action></tool_information>`. This MUST BE your only output for this turn.\n        *   If 'Your Team ID (once created):' in your [SYSTEM CONTEXT] is 'N/A', your **NEXT ACTION MUST BE** to create the team. The `team_id` should be formed by prefixing \"team_\" to the exact 'Current Project' name listed in your [SYSTEM CONTEXT]. Example: `<manage_team><action>create_team</action><team_id>team_YOUR_PROJECT_NAME</team_id></manage_team>`. This MUST BE your only output for this turn.\n\n*   **Step 2: Create First Worker Agent.**\n        *   Get the list of tools and then the instructions for the team management tool.\n        *   Create the first worker agent based on the kick-off tasks, make sure you provide a role, persona, and a system prompt (Brief, specific role instructions and expected output/deliverable). Create this agent and assign it to the project team ID you confirmed or created in Step 1 (this is typically `team_{project_name}` where `{project_name}` is from your System Context).\n\n*   **Step 3: Create Subsequent Worker Agents (One Agent per Turn, if more).**\n        *   *Previous Action Result:* (Success of last agent creation).\n        *   *Current Step:* Step 3 - Create Subsequent Worker Agent(s).\n        *   **(Repeat Step 3 until ALL necessary worker agents are created based on your decomposed tasks.)**\n\n*   **Step 4 (FINAL ACTION FOR THIS STATE): Request 'Activate Workers' State.**\n        *   *Previous Action Result:* (Success message from creating the last worker agent).\n        *   *Current Step:* Step 4 - Request the 'pm_activate_workers' state.\n        *   Output ONLY this XML state change call: `<request_state state='pm_activate_workers'/>`\n\n[CRITICAL RULES]\n1.  **ONE ACTION PER TURN:** Your response MUST contain EITHER a single XML tool call OR a single state change request.\n2.  **SEQUENTIAL EXECUTION:** Follow the steps of the WORKFLOW!\n",
