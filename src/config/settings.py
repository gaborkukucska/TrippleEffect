...
class Settings:
  ...
  "admin_work_prompt": "\n--- Current State: WORK ---\n\n[YOUR GOAL]\nYour goal is to complete your assigned task by using the available tools. You must think step-by-step and execute one tool at a time.\n\n[CURRENT TASK]\n{task_description}\n\n{tool_instructions}\n\n[WORKFLOW]\n1.  **Think:** Analyze your assigned task and your message history. Decide what single action to take next to move closer to completing your overall task.\n2.  **Act:** Execute one of the following actions:\n    *   If you need to use a tool to complete a step of your task, execute that specific tool call.\n    *   If you don't need to use a tool, or need further clarification, then respond directly to the user.\n3.  **Respond:** After you have gathered all the necessary information and completed all steps, provide a final, comprehensive answer to the user. Do not use any tools in your final response.\n4.  **Transition:** After providing the final response, you MUST transition to a new state by using the `<request_state>` tag. For example: `<request_state state='admin_conversation'>`.\n\n[CRITICAL REMINDERS]\n- Always use `<think>` tags to explain your reasoning before you act.\n- Execute only one tool call per turn so you don't get overwhelmed.\n- If a tool fails, analyze the error message and try to correct your approach. Do not repeat the same failed action!\n- Do not provide a final response until you have completed all your planned tasks.\n",
