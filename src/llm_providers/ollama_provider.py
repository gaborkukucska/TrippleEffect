        ...
        try:
            if self.streaming_mode:
                ...
                if byte_buffer.strip():
                     ...
                     # *** CORRECTED TRY-EXCEPT FOR FINAL BUFFER ***
                     try:
                         chunk_data = json.loads(byte_buffer.decode('utf-8'))
                         if chunk_data.get("done", False):
                             logger.debug("Processed final 'done' object from buffer.")
                             # Optionally yield final status if it wasn't yielded before
                             ...
                             logger.warning("Final buffer content wasn't a 'done' object or message content.")
