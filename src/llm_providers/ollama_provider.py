...
DEFAULT_READ_TIMEOUT = 300.0 # Keep long read timeout for chunks
...
class OllamaProvider(BaseLLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, **kwargs):
        # (init remains the same as previous httpx version)
        ...
        # Timeout configured in _get_client
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        # (remains the same as previous httpx version)
    ...
    async def stream_completion(
        ...
        **kwargs
        ...
        last_exception = None
        response_context = None # To hold the httpx response context manager

        # --- Retry Loop for Initial API Request (Unchanged) ---
        for attempt in range(MAX_RETRIES + 1):
            last_exception = None
            response_context = None # Reset context for each attempt
            try:
                 logger.info(f"OllamaProvider making API call (Attempt {attempt + 1}/{MAX_RETRIES + 1}).")
                 # Initiate the stream context
                 response_context = client.stream("POST", chat_endpoint, json=payload)
                 async with response_context as resp:
                    # --- Read only headers first to check status ---
                    await resp.aread_headers()
                    response_status = resp.status_code
                    response_text = "" # Reset response text for this attempt

                    if response_status >= 400:
                         try:
                             # Read the full error body now that headers are read
                    ...
                    # *** DO NOT break here yet, need the response object from the context manager ***
                    response = resp # Store the response from the active context
                    break # Break retry loop AFTER assigning response
                ...
                # Ensure context is closed if we continue/return within the loop on error
                if response is None and response_context is not None: # If we broke out of 'async with' early due to error/retry
                    await response_context.aclose()


        if response is None: # Check if we exited the loop without a successful response
        ...
        # --- Process the Successful Response Stream using aiter_raw ---
        byte_buffer = b""
        processed_lines = 0
        ...
        try:
            if self.streaming_mode:
                ...
                async for chunk in response.aiter_raw():
                    ...
                    # logger.log(logging.DEBUG - 1, f"Raw chunk received (len={len(chunk)}): {chunk[:100]}...")
                    byte_buffer += chunk
                    while True: # Process complete lines in buffer
                        ...
                        try:
                            ...
                            # Process parsed object (same logic as before)
                            ...
                            logger.error(f"JSONDecodeError line {processed_lines}: {decoded_line[:500]}..."); yield {"type": "error", "content": "[OllamaProvider Error]: Failed stream JSON decode."}; stream_error_occurred = True; break
                        except Exception as e:
                            logger.error(f"Error processing stream line {processed_lines}: {e}", exc_info=True); logger.error(f"Problem line: {decoded_line[:500]}"); yield {"type": "error", "content": f"[OllamaProvider Error]: Stream processing error - {type(e).__name__}"}; stream_error_occurred = True; break

                    if stream_error_occurred: break # Break outer async for loop

                logger.debug(f"Finished streaming loop (aiter_raw). Processed {processed_lines} lines. Error: {stream_error_occurred}")
                ...
                # (Non-streaming logic remains the same, uses response.aread())
                logger.debug("Processing non-streaming response...")
                try:
                    full_response_text = await response.aread() # Read full body
                    response_data = json.loads(full_response_text.decode('utf-8'))
                    # ... (rest of non-streaming logic) ...
        ...
        # --- Catch exceptions DURING stream processing ---
             ...
             # Explicitly catch the error we saw before
             logger.error(f"Ollama httpx stream closed unexpectedly: {stream_closed_err}", exc_info=True)
             yield {"type": "error", "content": f"[OllamaProvider Error]: Stream closed unexpectedly - {stream_closed_err}"}
             ...
             yield {"type": "error", "content": f"[OllamaProvider Error]: Connection closed unexpectedly - {proto_err}"}
        except httpx.NetworkError as net_err:
             logger.error(f"Ollama processing failed with NetworkError: {net_err}", exc_info=True)
             yield {"type": "error", "content": f"[OllamaProvider Error]: Network error during stream - {net_err}"}
        except Exception as e:
            ...
            # Ensure the response stream context is closed if it was opened
            if response is not None and not response.is_closed:
                 await response.aclose()
                 logger.debug("Explicitly closed httpx response stream.")
