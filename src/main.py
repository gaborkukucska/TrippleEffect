...
# --- Import the AgentManager class (Reverted to top-level) ---
logger.info("main.py: Attempting to import AgentManager...")
try:
    from src.agents.manager import AgentManager
    logger.info("main.py: Successfully imported AgentManager.")
except ImportError as e:
    logger.critical(f"main.py: FAILED to import AgentManager: {e}", exc_info=True)
    # Exit if critical import fails
    raise SystemExit("Failed to import critical AgentManager component.") from e
except Exception as e:
    logger.critical(f"main.py: An unexpected error occurred during AgentManager import: {e}", exc_info=True)
    raise SystemExit("Unexpected error during critical import.") from e
# --- End Import ---
...
# --- Type Hinting ---
if TYPE_CHECKING:
    from src.agents.manager import AgentManager # Keep for type hinting
...
async def lifespan(app: FastAPI):
    ...
    """
    # NOTE: AgentManager is now imported at the top level again
        ...
        if proxy_can_start:
            ...
            try:
                ollama_proxy_process = subprocess.Popen(
                    ...
                    env=proxy_env, start_new_session=True,
                ...
                max_wait_time = 15; check_interval = 0.5; start_time = time.monotonic()
                ...
                while time.monotonic() - start_time < max_wait_time:
                    if ollama_proxy_process.poll() is not None:
                        logger.error(f"Ollama proxy process (PID: {ollama_proxy_process.pid}) exited prematurely. Code: {ollama_proxy_process.returncode}")
                        try: logger.error(f"Proxy stderr: {ollama_proxy_process.stderr.read() if ollama_proxy_process.stderr else 'N/A'}")
                        except Exception as e: logger.error(f"Error reading proxy stderr: {e}")
                        break
                    try:
                        async with aiohttp.ClientSession() as session:
                            async with session.get(proxy_check_url, timeout=check_interval) as response:
                                ...
                                else: logger.debug(f"Proxy check attempt failed with status {response.status}. Retrying...")
                    except (aiohttp.ClientConnectorError, asyncio.TimeoutError): logger.debug(f"Proxy not yet reachable at {proxy_check_url}. Retrying...")
                    except Exception as check_err: logger.warning(f"Unexpected error during proxy readiness check: {check_err}")
                ...
                if not proxy_ready:
                    ...
                    if ollama_proxy_process and ollama_proxy_process.poll() is None:
                         ...
                         except Exception as term_err: logger.error(f"Error during proxy termination: {term_err}")
                    app.state.ollama_proxy_process = None
                    ollama_proxy_process = None
                else: logger.info("Ollama proxy appears to be running and ready.")
            except Exception as e:
                logger.error(f"Failed during Ollama proxy startup or readiness check: {e}", exc_info=True)
                ollama_proxy_process = None; app.state.ollama_proxy_process = None
    else:
    ...
    agent_manager_instance = AgentManager() # AgentManager is imported at top
    ...
    if proxy_process_to_stop and isinstance(proxy_process_to_stop, subprocess.Popen):
        ...
        if proxy_process_to_stop.poll() is None:
            try:
                ...
                if process_group_id: os.killpg(process_group_id, signal.SIGTERM); logger.info(f"Sent SIGTERM to process group {process_group_id}")
                else: proxy_process_to_stop.terminate(); logger.info(f"Sent terminate signal to process {proxy_process_to_stop.pid}")
                try: proxy_process_to_stop.wait(timeout=5); logger.info(f"Ollama proxy process {proxy_process_to_stop.pid} terminated gracefully.")
            ...
            except ProcessLookupError: logger.info(f"Ollama proxy process {proxy_process_to_stop.pid} already terminated.")
            except Exception as e: logger.error(f"Error stopping Ollama proxy process {proxy_process_to_stop.pid}: {e}", exc_info=True)
        else: logger.info(f"Ollama proxy process {proxy_process_to_stop.pid} was already stopped.")
        app.state.ollama_proxy_process = None
