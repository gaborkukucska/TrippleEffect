...
from src.agents.performance_tracker import ModelPerformanceTracker # For spec
from src.config.settings import Settings # For spec
from src.agents.provider_key_manager import ProviderKeyManager # For spec

# Disable most logging output for tests unless specifically testing logging
import logging
logging.disable(logging.CRITICAL)

class TestSelectBestAvailableModel(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.mock_performance_tracker = MagicMock(spec=ModelPerformanceTracker)
        self.manager_mock.performance_tracker = self.mock_performance_tracker
        
        self.settings_patcher = patch('src.agents.agent_lifecycle.settings')
        ...
        self.manager_mock.available_local_providers_list = {}
        ...
        # The _select_best_available_model transforms ModelPerformanceTracker's internal format.
        # We mock get_metrics on performance_tracker.
        self.mock_performance_tracker.get_metrics = MagicMock(return_value=self.perf_metrics)
    ...
    async def test_scenario_1_tier_all(self):
        ...
        provider, model_id, _, _ = await _select_best_available_model(self.manager_mock)
    ...
    async def test_scenario_6_models_without_size_or_performance(self):
        ...
        self.mock_performance_tracker.get_metrics = MagicMock(return_value={
            "openrouter": {
                "no_size_has_perf": {"score": 0.9}
            }
        })
